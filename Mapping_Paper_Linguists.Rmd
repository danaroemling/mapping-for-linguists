...
```{r join us data}
us_geo_data <- na.omit(left_join(us_geo, us_data, 
                         by = c("ID" = "county"))) 
```
...
```{r me class intervals assignment}
...
```{r me plot 2}
ggplot(data = us_geo_data) +
  geom_sf(aes(fill = pronoun_quantile),
          lwd = 0.1, 
          color = "grey") +
